...
class TracedAgenticPipeline(BasePipeline):
    ...
    @torch.no_grad()
    def run(self):
        # Tracing: initialize driver-side tracer
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            _step_span = tracer.span("pipeline.step", phase="pipeline", step=global_step, attrs={"global_step": global_step})
            _step_span.__enter__()
            with Timer(name="pipeline_step_total", logger=None) as step_timer:
                with tps_timer:
                    # PHASE 1: Offload States
                    _offload_train_span = tracer.span("weight_sync.offload_train", phase="weight_sync", step=global_step)
                    _offload_train_span.__enter__()
                    if self.pipeline_config.adv_estimator == "gae":
                        self.critic.offload_states(blocking=True)
                    self.actor_train.offload_states(blocking=True)
                    _offload_train_span.__exit__(None, None, None)

                    # PHASE 2: Suspend & Stop Server
                    # Suspend rollout scheduler to pause request processing
                    _suspend_for_train_span = tracer.span("scheduler.suspend_for_train", phase="scheduler", step=global_step)
                    _suspend_for_train_span.__enter__()
                    ray.get(self.train_rollout_scheduler.suspend.remote())
                    _suspend_for_train_span.__exit__(None, None, None)

                    # Stop generation server if using async mode (will restart after model update)
                    if self.pipeline_config.async_pipeline:
                        _offload_infer_span = tracer.span("weight_sync.offload_infer", phase="weight_sync", step=global_step)
                        _offload_infer_span.__enter__()
                        self.actor_infer.offload_states(include=OffloadStateType.other_params)
                        _offload_infer_span.__exit__(None, None, None)

                    # PHASE 3: Model Update
                    with Timer(name="model_update", logger=None) as model_update_timer:
                        _ws_span = tracer.span("weight_sync.sync_weights", phase="weight_sync", step=global_step)
                        _ws_span.__enter__()
                    ...
                    _load_infer_span = tracer.span("weight_sync.load_infer", phase="weight_sync", step=global_step)
                    _load_infer_span.__enter__()
                    self.actor_infer.load_states()
                    if self.reward:
                        self.reward.load_states()
                    _load_infer_span.__exit__(None, None, None)
                    ...
                    if self.partial_gpu_mode and global_step > 0:
                        ...
                        if target_gpus:
                            _expand_span = tracer.span("scheduler.expand", phase="scheduler", step=global_step)
                            _expand_span.__enter__()
                            expand_metrics = ray.get(
                                self.train_rollout_scheduler.expand_sampler.remote(target_gpus, skip_load=True)
                            )
                            _expand_span.__exit__(None, None, None)
                    ...
                    with Timer(name="val", logger=None) as val_timer:
                        ...
                        with Timer(name="rollout", logger=None) as rollout_timer:
                            _rollout_span = tracer.span("rollout.collect_batch", phase="rollout", step=global_step)
                            _rollout_span.__enter__()
                            _get_batch_span = tracer.span("rollout.get_batch", phase="rollout", step=global_step)
                            _get_batch_span.__enter__()
                            batch = ray.get(self.train_rollout_scheduler.get_batch.remote(batch, self.pipeline_config.rollout_batch_size))
                            _get_batch_span.__exit__(None, None, None)
                            ...
                            _rollout_span.__exit__(None, None, None)
                    ...
                    if not self.pipeline_config.async_pipeline:
                        ...
                        _suspend_span = tracer.span("scheduler.suspend", phase="scheduler", step=global_step)
                        _suspend_span.__enter__()
                        ray.get(self.train_rollout_scheduler.suspend.remote())
                        _suspend_span.__exit__(None, None, None)
                        _offload_infer_span = tracer.span("weight_sync.offload_infer", phase="weight_sync", step=global_step)
                        _offload_infer_span.__enter__()
                    ...
                    #   Next iteration: model_update reloads actor_infer to all GPUs [0,1,2,3]
                        ...
                        with Timer(name="cal_ref_log_probs", logger=None) as shrink_timer:
                            ...
                            _shrink_span = tracer.span("scheduler.shrink", phase="scheduler", step=global_step)
                            _shrink_span.__enter__()
                            shrink_metrics = ray.get(self.train_rollout_scheduler.shrink_sampler.remote(target_gpus))
                            _shrink_span.__exit__(None, None, None)
                    ...
                    with Timer(name="cal_ref_log_probs", logger=None) as cal_timer:
                        _ref_span = tracer.span("reward.ref_logprob", phase="reward", step=global_step)
                        _ref_span.__enter__()
                        ...
                        _ref_span.__exit__(None, None, None)
                    metrics["timing.reward.ref_logprob"] = cal_timer.last

                    # PHASE 12: Old Log Probs & Values
                    with Timer(name="cal_old_log_probs_values", logger=None) as cal_old_logpb_timer:
                        _pe_span = tracer.span("policy_eval.logprob", phase="policy_eval", step=global_step)
                        _pe_span.__enter__()
                        ...
                        _pe_span.__exit__(None, None, None)
                    ...
                    with Timer(name="compute_advantage", logger=None) as timer:
                        _adv_span = tracer.span("advantage.compute", phase="advantage", step=global_step)
                        _adv_span.__enter__()
                    ...
                    with Timer(name="train_timer", logger=None) as train_timer:
                        _train_span = tracer.span("training.actor_update", phase="training", step=global_step)
                        _train_span.__enter__()
                        if self.pipeline_config.adv_estimator == "gae":
                        ...
                        _train_span.__exit__(None, None, None)
